  ...
  public function comply(lhs:GComplexTypeT,rhs:GComplexTypeT){
    return switch([lhs,rhs]){
      ...
      case [GTExtend(pI,fieldsI) ,GTExtend(pII,fieldsII)]     : 
        var lset = RedBlackSet.make(Comparable.Anon(new GTypePath(),new stx.assert.ord.g.GTypePath()));
            lset = lset.concat(pI);
        var rset = RedBlackSet.make(Comparable.Anon(new GTypePath(),new stx.assert.ord.g.GTypePath()));
            rset = rset.concat(pII);
        var eq   = lset.equals(rset);
        if(eq.is_ok()){
          var lset = RedBlackSet.make(Comparable.Anon(new GField(),new stx.assert.ord.g.GField()));
            lset = lset.concat(fieldsI);
          var rset = RedBlackSet.make(Comparable.Anon(new GField(),new stx.assert.ord.g.GField()));
      ...
      case [GTIntersection(tlI),GTIntersection(tlII)]         : 
        var lset = RedBlackSet.make(Comparable.Anon(this,new stx.assert.ord.g.GComplexType()));
            lset = lset.concat(tlI);
        var rset = RedBlackSet.make(Comparable.Anon(this,new stx.assert.ord.g.GComplexType()));
            rset = rset.concat(tlII);
        lset.equals(rset);
      case [l,r]                                              : 
        Eq.EnumValue().comply(l,r);
    }
  }
}